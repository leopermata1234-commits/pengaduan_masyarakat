...
new #[Title('Tambah Data Warga')] class extends Component
{
    public string $name = '';
    ...
    {
        Gate::authorize('create.data.warga');
        ...
        DB::transaction(function () use ($validated): void {
            ...
            $user->assignRole(Role::findOrCreate('Masyarakat', 'web'));
        });

        session()->flash('success', __('Data warga berhasil ditambahkan.'));
...
<section class="mx-auto flex w-full max-w-5xl flex-col gap-6">
    <div class="flex flex-col gap-2">
        <div class="flex items-center gap-2 text-sm text-[#756858]">
            ...
            <span>/</span>
            <span class="font-semibold text-[#352b22]">{{ __('Tambah Warga') }}</span>
        </div>
        <h1 class="font-serif text-3xl font-bold text-[#2f241b]">{{ __('Tambah Data Warga') }}</h1>
        <p class="text-sm text-[#625b53]">{{ __('Masukkan identitas warga untuk melengkapi data masyarakat tanpa membuat akun login.') }}</p>
    </div>
    ...
    <form wire:submit="save" class="overflow-hidden rounded-2xl border border-[#d8c8b5] bg-[#f8f1e7] shadow-[0_10px_28px_rgba(62,44,29,.12)]">
        <div class="bg-[linear-gradient(135deg,#776352,#9b7b5a_55%,#6d5543)] px-6 py-4 text-white">
            <h2 class="font-bold">{{ __('Identitas Warga') }}</h2>
            <p class="mt-1 text-xs text-white/80">{{ __('Kolom bertanda wajib harus diisi dengan data yang benar.') }}</p>
        </div>

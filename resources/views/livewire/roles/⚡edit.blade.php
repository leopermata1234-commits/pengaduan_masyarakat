...
<section class="mx-auto flex w-full max-w-4xl flex-col gap-6">
    <div class="flex flex-col gap-2">
        <div class="flex items-center gap-2 text-sm text-zinc-500"><a href="{{ route('roles.index') }}" wire:navigate>{{ __('Roles') }}</a><span>/</span><span>{{ __('Edit') }}</span></div>
        <h1 class="text-2xl font-semibold text-zinc-950 dark:text-white">{{ __('Edit Role') }}</h1>
    </div>
